...
// Compute the eta of two muon four-vectors
float computeeta(RVec<float>& pt, RVec<float>& eta, RVec<float>& phi, RVec<float>& mass) {
    ROOT::Math::PtEtaPhiMVector eta1(pt[0], eta[0], phi[0], mass[0]);
    ROOT::Math::PtEtaPhiMVector eta2(pt[1], eta[1], phi[1], mass[1]);
...
void etast() {
    ...
    // Compute eta of the dimuon system
   
    
      auto df_eta = df_os.Define("Dimuon_eta", computeeta,
    ...
    auto hist = df_eta.Histo1D({"", "", bins, low, up}, "Dimuon_eta");
    
    
    

    // Request cut-flow report
    auto report = df_eta.Report();
