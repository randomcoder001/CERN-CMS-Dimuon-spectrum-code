...
using namespace ROOT::VecOps;
float checking(RVec<int>& chargee,RVec<float>& ptt)
{
...
if (chargee[1]==1)
 ...
 void ptpos() {
  ...
  auto df_he=df_os.Define("mUon_pt",checking,{"Muon_charge","Muon_pt"});
   ...
   //Book histogram for muon_pt
     const auto bins = 1000; // Number of bins in the histogram
     const auto low =0; // Lower edge of the histogram
     const auto up = 100; // Upper edge of the histogram
     auto hist=df_he.Histo1D({"","",bins,low,up},"mUon_pt");
    ...
    // Draw histogram
    hist->GetXaxis()->SetTitle("pt of #mu^{+}");
