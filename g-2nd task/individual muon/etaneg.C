...
float checking(RVec<int>& chargee,RVec<float>& etaa)
{
...
if (chargee[1]==-1)
 ...
 void etaneg() {
  ...
  auto df_he=df_os.Define("mUon_eta",checking,{"Muon_charge","Muon_eta"});
  
       
   //Book histogram for NEG muon_eta
     const auto bins = 3000; // Number of bins in the histogram
     const auto low = -2.4; // Lower edge of the histogram
     const auto up = 2.4; // Upper edge of the histogram
     auto hist=df_he.Histo1D({"","",bins,low,up},"mUon_eta");
    ...
    // Create canvas for plotting muoneta
    ...
    // Draw histogram
    hist->GetXaxis()->SetTitle("eta of #mu^{-}");
