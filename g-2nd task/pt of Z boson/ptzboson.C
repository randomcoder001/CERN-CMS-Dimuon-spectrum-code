//code for pt of Z Boson using the invariant mass cut
...
// Compute the pt of two muon four-vectors
float computept(RVec<float>& pt, RVec<float>& eta, RVec<float>& phi, RVec<float>& mass) {
    ROOT::Math::PtEtaPhiMVector pt1(pt[0], eta[0], phi[0], mass[0]);
    ROOT::Math::PtEtaPhiMVector pt2(pt[1], eta[1], phi[1], mass[1]);
...
void ptzboson() {
    ...
    auto df_mass = df_os.Define("Dimuon_mass", computeInvariantMass,
                                {"Muon_pt", "Muon_eta", "Muon_phi", "Muon_mass"}).Filter("80.0<Dimuon_mass<96.0");
    
 
   
    
      auto df_pt = df_mass.Define("Dimuon_pt", computept,
                                {"Muon_pt", "Muon_eta", "Muon_phi", "Muon_mass"});
      
 
         
          
    // Book histogram of dimuon pt
    const auto bins = 600; // Number of bins in the histogram
    const auto low = 0; // Lower edge of the histogram
    const auto up = 100; // Upper edge of the histogram
    auto hist = df_pt.Histo1D({"", "", bins, low, up}, "Dimuon_pt");
    
    
    

    // Request cut-flow report
    auto report = df_pt.Report();
    ...
    // Draw histogram
    hist->GetXaxis()->SetTitle("pt_zboson");
